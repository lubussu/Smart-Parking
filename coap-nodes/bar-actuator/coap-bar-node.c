...
// Define the resource
extern coap_resource_t res_bar;
...
PROCESS(coap_bar_node, "CoAP Bar Node");
AUTOSTART_PROCESSES(&coap_bar_node);

PROCESS_THREAD(coap_bar_node, ev, data){
    PROCESS_BEGIN();
    ...
    coap_activate_resource(&res_bar, "bar");
    leds_on(LEDS_GREEN);
