...
#define NOX_THRESHOLD_RANGE 50
#define NOX_UPPER_BOUND 192
#define NOX_LOWER_BOUND 8
#define NOX_STD 9
static int ppb = 0;
...
static void sensor_sample(){
	...
	if(state == STATE_CONNECTED){
		// Publish something
		sprintf(pub_topic, "%s", "nox");

		// sense the NOx level in Points Per Billion (ppb)
		if(((rand() % 2) == 0)){
			if(ppb < NOX_UPPER_BOUND){
				ppb += rand() % NOX_STD;
			}
		}else{
			if(ppb > NOX_LOWER_BOUND){
				ppb -= rand() % NOX_STD;
			}
		}
...
static void inc_range(){
	if(range < 3){
		range++;
		ppb = range * NOX_THRESHOLD_RANGE + (rand() % NOX_THRESHOLD_RANGE - 1);
		leds_off(14);
		leds_on(1 << range);
	}
}
